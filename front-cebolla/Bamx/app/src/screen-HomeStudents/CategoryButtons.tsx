...
const CategoryButtons: React.FC = () => {
  ...
  const renderDonationCard = ({ item }: { item: any }) => {
    const imageUri = item.img || fallbackImage; // Use fallback if `item.img` is null or undefined
    ...
    return (
      <View style={donationStyles.card}>
        <Image
          ...
          onError={() => (item.img = fallbackImage)} // Update the `item.img` to fallback when error occurs
        />
