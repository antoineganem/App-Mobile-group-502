...
# Endpoint to search for activities the student is  involved in
...
def search_student():
    ...
    query_total_hours = """
        SELECT 
        COALESCE((SELECT SUM(hours) FROM hours_activities WHERE id_student = %s AND status = TRUE), 0) +
        COALESCE((SELECT SUM(hours) FROM hours_donations WHERE id_student = %s AND status = TRUE), 0) AS total_hours;
    """
    ...
    query_activities = """
    SELECT A.id, A.hours, A.description, A.date, HA.status
    FROM activities A 
    ...
    query_donations = """
    SELECT D.id, D.hours, D.name, D.due_date, HD.status
    FROM donations D 
